...
bot.on("message", msg => {
    ...
    if (msg.content.toLowerCase().startsWith(prefix + "ping")) {
        ...
        translate(msg.content.split(" ").slice(3).join(" "), {
            from: msg.content.split(" ").slice(1, 2).join(" "),
            to: msg.content.split(" ").slice(2, 3).join(" ")
        }).then(res => {
            msg.reply(res.text);
        }).catch(err => {
            console.error(err);
        ...
        if (pkmn.getPokemonIdByName(msg.content.split(" ").slice(1, 2).join(" "))) {
            msg.channel.sendFile(`${pokemonGif(msg.content.split(" ").slice(1, 2).join(" "))}`, ".gif", "#" +
                ...
                baseStats.getByName({
                    name: msg.content.split(" ").slice(1, 2).join(" ")
                }) +
...
bot.on('message', msg => {
    ...
    if (msg.content.toLowerCase() === ("roll d20") ||
        ...
        let rnumber = Math.floor((Math.random() * xrandom) + 1)
        switch (rnumber) {
            case 1:
            ...
            case 20:
                ...
                break;
            default:
                msg.reply("you rolled a " + rnumber);
        }
    }
});
